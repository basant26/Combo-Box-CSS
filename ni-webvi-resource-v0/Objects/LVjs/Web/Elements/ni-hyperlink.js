...
JQX('ni-hyperlink', class Hyperlink extends JQX.BaseElement {
    ...
    template() {
        return `<a rel="noopener"></a>`;
    }
    /** Called when the element is ready. Used for one-time configuration of the Hyperlink. */
    ready() {
        const that = this;
        super.ready();
        const anchorElement = that.firstElementChild;
        if (!that.disabled) {
            anchorElement.setAttribute('href', that.href);
        }
        anchorElement.target = that.target;
        anchorElement.textContent = that.content;
    }
    /**
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            case 'href':
                ...
                break;
            case 'target':
                childElement.target = that.target;
                break;
            case 'content':
