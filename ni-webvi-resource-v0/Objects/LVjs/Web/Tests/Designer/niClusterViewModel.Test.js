...
describe('A ClusterViewModel', function () {
    ...
    let controlId, clusterSettings, innerClusterSettings, innerClusterLabelSettings, clusterContainingNumericSettings, clusterContainingNumericLabelSettings, clusterContainingStringSettings, clusterContainingStringLabelSettings, clusterContainingColorBoxSettings, clusterContainingColorBoxLableSettings, clusterContainingComboBoxLableSettings, clusterContainingComboBoxSettings, viewModel, newNIType, element;
    beforeAll(function (done) {
        ...
        testHelpers.fetchJsonFixtureForElements().then((fixture) => {
            ...
            clusterContainingColorBoxSettings = fixture.clusterContainingColorBoxSettings;
            clusterContainingColorBoxLableSettings = fixture.clusterContainingColorBoxLableSettings;
            clusterContainingComboBoxLableSettings = fixture.clusterContainingComboBoxLableSettings;
            clusterContainingComboBoxSettings = fixture.clusterContainingComboBoxSettings;
    ...
    describe('allows dropping of combo box ', function () {
        beforeEach(function (done) {
            webAppHelper.createNIElementHierarchy([
                { currentSettings: clusterSettings },
                { currentSettings: clusterContainingComboBoxLableSettings, parentId: controlId },
                { currentSettings: clusterContainingComboBoxSettings, parentId: controlId }
            ], done);
        });
        beforeEach(function () {
            ...
            newNIType = new NIType({
                ...
                fields: [clusterContainingComboBoxLableSettings.text],
        ...
        it('and updates the element when properties change.', function (done) {
            const field = clusterContainingComboBoxLableSettings.text;
            const newValue = {};
            testHelpers.runMultipleAsync(done, function () {
                newValue[field] = 'New_Test';
                webAppHelper.dispatchMessage(controlId, {
                    niType: newNIType,
                    value: newValue
                });
            }, function () {
                const comboBox = element.querySelectorAll('jqx-combo-box')[0];
                expect(element.value).toEqual(newValue);
                expect(comboBox.value).toEqual(newValue[field]);
            });
        });
    });
    describe('create nested cluster ', function () {
        ...
        it('set cluster size wide enough for string to be visible and set size to narrow enough for string control to be hidden', function (done) {
    ...
    describe('allows dropping of color box ', function () {
        beforeEach(function (done) {
            webAppHelper.createNIElementHierarchy([
                { currentSettings: clusterSettings },
                { currentSettings: clusterContainingColorBoxLableSettings, parentId: controlId },
                { currentSettings: clusterContainingColorBoxSettings, parentId: controlId }
            ], done);
        });
        beforeEach(function () {
            viewModel = viModel.getControlViewModel(controlId);
            ...
            newNIType = new NIType({
                name: NITypeNames.CLUSTER,
                fields: [clusterContainingColorBoxLableSettings.text],
                subtype: [NITypeNames.UINT32]
            }).toJSON();
        });
        afterEach(function () {
            webAppHelper.removeNIElement(controlId);
        ...
        it('and updates the element when properties change.', function (done) {
            const field = clusterContainingColorBoxLableSettings.text;
            const newValue = {};
            testHelpers.runMultipleAsync(done, function () {
                newValue[field] = 45454334;
                webAppHelper.dispatchMessage(controlId, {
                    niType: newNIType,
                    value: newValue
                });
            }, function () {
                const colorBox = element.querySelectorAll('jqx-color-picker')[0];
                expect(element.value).toEqual(newValue);
                expect(colorBox.value).toEqual(ColorBoxValueConverter.convert(newValue[field]));
            });
        });
    });
});
