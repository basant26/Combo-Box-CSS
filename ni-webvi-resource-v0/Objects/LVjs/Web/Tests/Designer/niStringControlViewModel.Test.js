...
describe('A StringControlViewModel', function () {
    ...
    describe('handles method node calls', function () {
        let viewModel, internalControl;
        beforeEach(function (done) {
            webAppHelper.createNIElement(stringSettings);
            testHelpers.runAsync(done, function () {
        ...
        it('handles \'scrollToEnd\'.', function (done) {
            makeAsync(done, async function () {
                controlModel.height = 66;
                controlModel.text = 'This is a very long text value. We are putting each word on its own line. Evenifthereisaverylongwordwhichcauseshorizontalscrolling, we should scroll to the bottom only. THIS IS VISIBLE'.replace(/ /g, '\n');
                await viewModel.invokeInternalControlFunction('scrollToEnd');
                expect(internalControl.scrollTop).toEqual(internalControl.scrollHeight - internalControl.clientHeight);
                done();
            });
        });
    });
