...
describe('A ArrayViewerViewModel', function () {
    ...
    beforeAll(function (done) {
        ...
        testHelpers.fetchJsonFixtureForElements().then((fixture) => {
            ...
            arrayViewerWithColorBoxSettings = fixture.arrayViewerWithColorBoxSettings;
    ...
    describe('allows dropping color box control', function () {
        beforeEach(function (done) {
            arrayViewerElement = webAppHelper.createNIElement(arrayViewerSettings);
            testHelpers.runMultipleAsync(done, function () {
                webAppHelper.createNIElement(arrayViewerWithColorBoxSettings, controlId);
            }, function () {
                frontPanelControls = viModel.getAllControlModels();
                arrayViewerModel = frontPanelControls[controlId];
            });
        ...
        it('and updates value property to element', function (done) {
            testHelpers.runMultipleAsync(done, function () {
                arrayViewerModel.rowsAndColumns = '3,1';
                arrayViewerModel.value = [0x02020304, 0x59356678, 0x00445600];
            }, function () {
                const childColorBoxes = arrayViewerElement.childElement.querySelectorAll('jqx-color-picker');
                childColorBoxes.forEach((colorBox, i) => {
                    expect(colorBox.value).toEqual(ColorBoxValueConverter.convert(arrayViewerModel.value[i]));
                });
            });
        });
    });
    describe('allows dropping combo box control', function () {
        beforeEach(function (done) {
            arrayViewerElement = webAppHelper.createNIElement(arrayViewerSettings);
            testHelpers.runMultipleAsync(done, function () {
                webAppHelper.createNIElement(comboBoxSelectorSettings, controlId);
            }, function () {
                frontPanelControls = viModel.getAllControlModels();
                arrayViewerModel = frontPanelControls[controlId];
            });
        ...
        it('and updates value property to element', function (done) {
            testHelpers.runMultipleAsync(done, function () {
                arrayViewerModel.rowsAndColumns = '3,1';
                arrayViewerModel.value = ['New_Test', 'New_Test', 'New_Test'];
            }, function () {
                const childComboBoxes = arrayViewerElement.childElement.querySelectorAll('jqx-combo-box');
                childComboBoxes.forEach((comboBox, i) => {
                    expect(comboBox.value).toEqual(arrayViewerModel.value[i]);
                });
            });
        });
    });
