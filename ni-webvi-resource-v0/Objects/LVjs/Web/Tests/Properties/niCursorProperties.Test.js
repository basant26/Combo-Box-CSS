...
describe('A CartesianGraph control with a cursor element', function () {
    ...
    describe('and a cursor legend', function () {
        const cursorLegendId = NI_SUPPORT.uniqueId();
        let cursorLegendElement;
        beforeEach(function (done) {
            const cursorLegendSettings = {
                niControlId: cursorLegendId,
                graphRef: graphId,
            ...
            webAppHelper.createNIElement(cursorLegendSettings);
            testHelpers.runAsync(done, function () {
                cursorLegendElement = viModel.getControlViewModel(cursorLegendSettings.niControlId).element;
            });
        });
        afterEach(function () {
            webAppHelper.removeNIElement(cursorLegendId);
        });
        it('property get for active cursor for a dynamically-added (via legend) cursor returns the new active cursor that is set.', function (done) {
            makeAsync(done, async function () {
                const cursor1Reference = graphViewModel.getGPropertyValue(GraphBaseModel.CURSOR_G_PROPERTY_NAME);
                expect(cursor1Reference).toBeDefined();
                cursorLegendElement.helpers.addCursor();
                await testHelpers.waitAsync();
                graphViewModel.setGPropertyValue(GraphBaseModel.ACTIVE_CURSOR_G_PROPERTY_NAME, 1);
                await testHelpers.waitAsync();
                expect(graphViewModel.getGPropertyValue(GraphBaseModel.ACTIVE_CURSOR_G_PROPERTY_NAME)).toEqual(1);
                const cursor2Reference = graphViewModel.getGPropertyValue(GraphBaseModel.CURSOR_G_PROPERTY_NAME);
                expect(cursor2Reference).toBeDefined();
                expect(cursor2Reference).not.toBe(cursor1Reference);
            });
        });
    });
