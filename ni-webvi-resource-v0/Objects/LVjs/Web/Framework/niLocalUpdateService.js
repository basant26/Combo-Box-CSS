...
export class LocalUpdateService extends UpdateService {
    ...
    static controlTestNeedsUpdateAndReset(vireo, controlModel) {
        const localBindingInfo = controlModel.getLocalBindingInfo();
        if (localBindingInfo === undefined) {
            return false;
        }
        const valueRef = vireo.eggShell.findValueRef(localBindingInfo.encodedVIName, localBindingInfo.runtimePath);
        const needsUpdate = vireo.eggShell.testNeedsUpdateAndReset(valueRef);
        return needsUpdate;
    }
    /**
    ...
    updateHTMLControls() {
        ...
        LocalUpdateService.forEachControlModelInEachVIModel(that.getVIModels(), function (viModel, controlModel) {
            const localBindingInfo = controlModel.getLocalBindingInfo();
            if (LocalUpdateService.controlAcceptsDiagramUpdates(controlModel) &&
                localBindingInfo.sync === false &&
                LocalUpdateService.controlTestNeedsUpdateAndReset(that.vireo, controlModel)) {
                const peekResult = VIREO_PEEKER.peek(that.vireo, localBindingInfo.encodedVIName, localBindingInfo.runtimePath);
                const messageData = {};
                messageData[localBindingInfo.prop] = peekResult;
