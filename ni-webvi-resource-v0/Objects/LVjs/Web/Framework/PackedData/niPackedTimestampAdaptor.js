...
export class NIPackedTimestampAdaptor {
    ...
    static UnpackFromData(timestampValue) {
        const seconds = timestampValue.seconds().toFloat64();
        const fractions = NIPackedLongUtils.ulongToFloat64(timestampValue.fractions());
        return new window.NITimestamp(seconds.toString() + ":" + fractions.toString());
    }
    /**
    ...
    static PackDataValue(builder, timestampDataToPack) {
        ...
        const fractions = NIPackedTimestampAdaptor.CreateFlatbuffersLong(timestampDataToPack.fractions * 0xFFFFFFFFFFFFFFFF);
        NIFlatBuffers.PackedData.PackedTimestampValue.addFractions(builder, fractions);
