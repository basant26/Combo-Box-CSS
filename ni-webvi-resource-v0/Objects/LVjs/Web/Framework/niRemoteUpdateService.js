...
export class RemoteUpdateService extends UpdateService {
    ...
    async handleRemotePanelUpdate(updateData) {
        ...
        if (requestType === NIFlatBuffersRemote.Web.RemotePanel.RemotePanelUpdateType.SetPropertyValue) {
            ...
            await this.setGPropertyValueAsync(setPropertyData.docName(), setPropertyData.runtimeId(), setPropertyData.propertyName(), setPropertyData.propertyJson());
        }
    ...
    async setGPropertyValueAsync(viName, controlId, propertyName, propertyValue) {
        try {
            const viModel = this.getVIModelByName(viName);
            await viModel.processControlUpdateToSetGPropertyValueAsync(controlId, propertyName, JSON.parse(propertyValue));
        }
        catch (error) {
            /*
                * Currently we are swallowing the errors(LabVIEWPropertyError) thrown by property get and set methods.
    ...
    async batchUpdateFromBufferAsync(batchItem, viName) {
        ...
        await viModel.processControlUpdateToSetGPropertyValueAsync(controlId, propertyName, gPropertyValue);
    }
    async batchUpdateFromOffsetAsync(batchItem, viName) {
        const propertyName = batchItem.propertyName();
        const controlId = batchItem.runtimeId();
    ...
    async batchUpdateFromJsonAsync(batchItem, viName) {
        const propertyName = batchItem.propertyName();
        const controlId = batchItem.runtimeId();
        ...
        const viModel = this.getVIModels()[viName];
        await viModel.processControlUpdateToSetGPropertyValueAsync(controlId, propertyName, value);
    }
