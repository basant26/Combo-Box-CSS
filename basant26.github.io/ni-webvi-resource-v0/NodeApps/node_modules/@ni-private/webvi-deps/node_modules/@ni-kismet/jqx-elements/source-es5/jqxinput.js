...
JQX('jqx-input', function (_JQX$BaseElement) {
    ...
    babelHelpers.createClass(Input, [{
        ...
        value: function _performSelect() {
            ...
            that.$.fireEvent('change', { value: value, label: label, oldValue: oldValue, oldLabel: oldLabel }, that.isInShadowDOM ? { composed: true, bubbles: true, cancelable: true } : undefined);
            that.close();
        }
    }, {
        key: '_offsetTop',
        ...
        value: function _open() {
            ...
            requestAnimationFrame(function () {
                var dropDownWidth = that.dropDownWidth;
                ...
                if (dropDownWidth && typeof dropDownWidth === 'string' && dropDownWidth.indexOf('%') !== -1) {
                    var fraction = parseFloat(dropDownWidth) / 100;

                    that.$.scrollView.style.setProperty('--jqx-input-drop-down-menu-width', that.offsetWidth * fraction + 'px');
                } else if (dropDownWidth !== 'auto' && dropDownWidth) {
                    that.$.scrollView.style.setProperty('--jqx-input-drop-down-menu-width', parseFloat(dropDownWidth) + 'px');
                ...
                if ((that.shadowRoot || that.isInShadowDOM) && that.$.scrollView._scrollView && JQX.ScrollBar) {
                    requestAnimationFrame(function () {
                        return that.$.scrollView._scrollView.vScrollBar.refresh();
                    });
                }
            });
