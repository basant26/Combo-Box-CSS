...
describe('A UrlImageViewModel', function () {
    ...
    describe('dynamically updates properties triggering ModelPropertyChanged', function () {
        let viewModel, internalControl, internalAnchor;
        ...
        it('updates multiple properties', function (done) {
            ...
            testHelpers.runAsync(fakeDone, function () {
                internalControl = controlElement.querySelector('.ni-image-box');
        ...
        it('Contains anchor with correct href if URL is set', function (done) {
            const url = 'http://www.ni.com/';
            webAppHelper.dispatchMessage(controlId, { 'href': url });
            testHelpers.runAsync(done, function () {
                internalAnchor = controlElement.querySelector('.ni-image-link');
                expect(internalAnchor.href).toEqual(url);
            });
        });
        it('Contains anchor with correct href and target if URL is set', function (done) {
            const url = 'http://www.ni.com/';
            const target = '_parent';
            webAppHelper.dispatchMessage(controlId, { 'href': url, target: target });
            testHelpers.runAsync(done, function () {
                internalAnchor = controlElement.querySelector('.ni-image-link');
                expect(internalAnchor.href).toEqual(url);
                expect(internalAnchor.target).toEqual(target);
            });
        });
        it('Contains no anchor if URL is not set', function () {
            internalAnchor = controlElement.querySelector('.ni-image-link');
            expect(internalAnchor).toEqual(null);
        });
