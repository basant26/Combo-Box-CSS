...
export class VirtualInstrumentModel extends NIModel {
    ...
    */
    ...
    async processControlUpdateToSetGPropertyValueAsync(controlId, gPropertyName, gPropertyValue) {
        const controlViewModel = this.controlViewModels[controlId];
        if (controlViewModel !== undefined) {
            if (controlViewModel.shouldUseAsyncSetGPropertyValue(gPropertyName)) {
                await controlViewModel.setGPropertyValueAsync(gPropertyName, gPropertyValue);
            }
            else {
    ...
    isInIdeMode() {
        const webAppModel = this.getOwningWebApplication();
        return webAppModel.isInIdeMode();
    }
    ensureFocus() {
        const webAppModel = this.getOwningWebApplication();
        webAppModel.ensureFocus();
    }
}
